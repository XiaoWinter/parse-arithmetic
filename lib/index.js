...
var ExpressBasicType;
(function (ExpressBasicType) {
    ExpressBasicType["ALPHABET"] = "ALPHABET";
    ExpressBasicType["LEFT_PARENTHESES"] = "LEFT_PARENTHESES";
    ExpressBasicType["RIGHT_PARENTHESES"] = "RIGHT_PARENTHESES";
    ExpressBasicType["SUBTRACT"] = "SUBTRACT";
})(ExpressBasicType || (ExpressBasicType = {}));
const TOKEN_MAPS = {
    ALPHABET: {
 ...
 * @return {*} 0 a优先级等于b
 */
function subtractPriority(a, b) {
    if (a.lexeme.match(SUBTRACT_PRIORITY.MUL_DIV.pattern) &&
        b.lexeme.match(SUBTRACT_PRIORITY.ADD_SUB.pattern))
        return 1;
    if (a.lexeme.match(SUBTRACT_PRIORITY.ADD_SUB.pattern) &&
        b.lexeme.match(SUBTRACT_PRIORITY.MUL_DIV.pattern))
        return -1;
    return 0;
}
// 中缀表达式转化为逆波兰表达式
function translateExpress2RPN(tokens) {
    const stack = [];
    const RPNExpress = [];
